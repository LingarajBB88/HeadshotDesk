    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: "Changing the schedule later", def: "Bookings that still fit the new schedule are kept. Extending the day or adding slots never touches anyone's time; click the + at the end of the Bookings grid to append slots one at a time. Only when a change would remove a booked time (shrinking the day, moving the grid, changing the date) does the app ask you to confirm, and it cancels just those bookings. The Reset button puts the settings back to what was last saved." },
          { term: "Removing and adding individual slots", def: "Hover an open slot in the Bookings grid and click the small x to take it off the schedule, for example around a meeting nobody told you about. Removed slots are listed under the grid; click one to restore it. The + chip at the end of the grid appends one slot per click. Both save immediately. Booked slots can't be removed directly: move or clear the booking from the Participants table first." },
        ],
      },
      {
        id: "participant-side",
